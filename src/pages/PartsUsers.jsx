...
export default function PartsUsers() {
  ...
  async function togglePermission(userId, perm, checked, table = "user_permissions", field = "permissions") {
    const key = `${userId}:${table}:${perm}`;
    setBusyKey(key);
    const { error } = checked
      ? await supabase.from(table).insert({ user_id: userId, permission: perm })
      : await supabase.from(table).delete().eq("user_id", userId).eq("permission", perm);
    setBusyKey("");
    if (error) {
                  ...
                  <tr key={u.id} className="hover:bg-ink-900/[0.015]">
                    ...
                    {PERMISSION_LABELS.map((p) => (
                      <td key={p.key} className="px-3 py-2.5 text-center">
                        <input
                          ...
                          disabled={u.is_admin || busyKey === `${u.id}:user_permissions:${p.key}`}
                    ...
                    {PALLET_PERMISSION_LABELS.map((p) => (
                      <td key={p.key} className="px-3 py-2.5 text-center">
                        <input
                          type="checkbox"
                          checked={u.is_admin || u.palletPermissions.has(p.key)}
